...
use fnv::FnvHasher;
use stopwords::{Language, Spark, Stopwords, NLTK};
...
type StopWordHashSet = HashSet<&'static str, StopWordHasher>;

/// `TokenFilter` that removes stop words from a token stream
#[derive(Clone)]
pub struct StopWordFilter {
    words: Arc<StopWordHashSet>,
}

impl StopWordFilter {
    /// Creates a `StopWordFilter` given a list of words to remove
    pub fn new(words: impl Iterator<Item = &'static str>) -> StopWordFilter {
        StopWordFilter {
            words: Arc::new(words.collect()),
        }
    }

    fn all_languages() -> StopWordFilter {
        use Language::*;
...
impl<'a> StopWordFilterStream<'a> {
    fn predicate(&self, token: &Token) -> bool {
        !self.words.contains(&token.text.as_str())
    }
}
